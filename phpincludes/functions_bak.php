<?php
	...
	if($serv == 1) {
		...
		$dbname="weblamosca";
		$imgroot = "/novaweb/img/";
		$imgrootsrv = "/usr/home/lamosca.com/web/novaweb/img/";
		$myServer = "http://www.lamosca.com/novaweb/";
	} else if($serv == 11) {
		$db_host="localhost"; 
		$db_user="root";
		$db_password="";
	...
	function buildModules($curl,$purl) {

		global $categorytable, $projecttable, $moduletable, $dbname, $imgroot, $imgrootsrv, $flashJS;
	...
	function buildIndex() {
	   ...
	   global $projecttable, $categorytable, $moduletable, $dbname, $imgroot, $imgrootsrv, $flashJS, $htmlnav;
		...
		if($modules[0]) {
			...
			$sqlstr = "SELECT id,title,image,imagetype,width,height,text_1,text_2,text_3,text_4 FROM $moduletable WHERE id = " . $modules[0];
			$erg = mysql_db_query($dbname, $sqlstr);
			$row=mysql_fetch_row($erg);
			$title = $row[1];
			$image = $row[2];
